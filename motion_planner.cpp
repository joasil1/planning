...
std::vector<State> MotionPlanner::generate_offset_goals(
  ...
  auto yaw = goal_state.rotation.yaw + M_PI/2;  // done <- Fix This
  ...
  for (int i = 0; i < _num_paths; ++i) {
    ...
    goal_offset.location.x += offset*cos(yaw);  // done <- Fix This
    goal_offset.location.y += offset*sin(yaw);  // done <- Fix This
