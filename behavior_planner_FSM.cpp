...
double BehaviorPlannerFSM::get_look_ahead_distance(const State& ego_state) {
  ...
  auto look_ahead_distance = 0.0;  // done <- Fix This
  // distance is given using the formula: x = x0+v0*t+0.5*a*t*t <=> delta_x = v0*t+0.5*a*t*t
  // we will need to compute 2 distances: reaction distance + braking distance.
  // for simplicity: we will consider only longitudinal movement
  // --- so, as a worst case estimate, we can use speed magnitude
  // for simplicity, we will assume that the acceleration is positive (we are accelerating instead of breaking)
  // --- this means that possibily we will estimate a larger look ahead distance than needed in case acceleration was negative, but it is okay
  // --- so, as a worst case estimate, we can use acceleration magnitude
  
  // reaction distance: keeps current motion (acceleration) during the reaction time
  look_ahead_distance += velocity_mag*_reaction_time+0.5*accel_mag*_reaction_time*_reaction_time;
  
  // braking distance:
  // speed after reaction time
  // speed is given using the formula: v = v0+a*t
  auto speed_after_reaction = velocity_mag+accel_mag*_reaction_time;
  // time to break is calculated using the max allowed acceleration and bringing speed to 0
  // time is given using formula 0 = v0+a*t <=> t = -v0/a <=> t = v0/a_break (assuming breaking acceleration is given in magnitude)
  auto time_to_break = speed_after_reaction/_max_accel;
  
  look_ahead_distance += speed_after_reaction*time_to_break+0.5*_max_accel*time_to_break*time_to_break;
...
State BehaviorPlannerFSM::state_transition(const State& ego_state, State goal,
  ...
  if (_active_maneuver == FOLLOW_LANE) {
    ...
    if (is_goal_in_junction) {
      ...
      goal.location.x += _stop_line_buffer*cos(ang);  // done <- Fix This
      goal.location.y += _stop_line_buffer*sin(ang);  // done <- Fix This

      LOG(INFO) << "BP- new STOP goal at: " << goal.location.x << ", "
                << goal.location.y;

      // TODO-goal speed at stopping point: What should be the goal speed??
      goal.velocity.x = 0.0;  // done <- Fix This
      goal.velocity.y = 0.0;  // done <- Fix This
      goal.velocity.z = 0.0;  // done <- Fix This
      ...
      goal.velocity.x = _speed_limit*cos(goal.rotation.yaw); // done <- Fix This
      goal.velocity.y = _speed_limit*sin(goal.rotation.yaw); // done <- Fix This
      goal.velocity.z = 0;
    }
    ...
    //    _stop_threshold_speed) {  // done -> Fix this
    if (distance_to_stop_sign <= P_STOP_THRESHOLD_DISTANCE) {
      ...
      _active_maneuver = STOPPED;  // done <- Fix This
    ...
    // the stop line. goal = ...;
    goal = _goal;  // Keep previous goal. Stay where you are. // done <- Fix This
    ...
    if (stopped_secs >= _req_stop_time && tl_state.compare("Red") != 0) {
      ...
      _active_maneuver = FOLLOW_LANE;  // done <- Fix This
