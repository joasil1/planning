...
double collision_circles_cost_spiral(const std::vector<PathPoint>& spiral,
  ...
  for (auto wp : spiral) {
    ...
    for (size_t c = 0; c < n_circles && !collision; ++c) {
      ...
      auto circle_center_x = cur_x+CIRCLE_OFFSETS[c]*cos(cur_yaw);  // done <- Update 
      auto circle_center_y = cur_y+CIRCLE_OFFSETS[c]*sin(cur_yaw);  // done <- Update 
      ...
      for (auto obst : obstacles) {
        ...
        for (size_t c2 = 0; c2 < n_circles && !collision; ++c2) {
          ...
          // obstacle/actor
          auto dx = circle_center_x-obst.location.x;
          auto dy = circle_center_y-obst.location.y;
          double dist = sqrt(dx*dx+dy*dy);  // done <- Update
...
double close_to_main_goal_cost_spiral(const std::vector<PathPoint>& spiral,
  ...
  auto delta_x = main_goal.location.x - spiral[n - 1].x;  // done <- Update
  auto delta_y = main_goal.location.y - spiral[n - 1].y;  // done <- Update
  auto delta_z = main_goal.location.z - spiral[n - 1].z;  // done <- Update
